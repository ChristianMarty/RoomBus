...
const tsp_triggerSignal_t triggerSignals[] = {
	...
	{ 6, LOADSWITCH_ADDRESS, 0x06, "Button 1"},
	{ 7, LOADSWITCH_ADDRESS, 0x09, "Button 2"},
	{ 8, LOADSWITCH_ADDRESS, 0x0C, "Button 3"},
	{ 9, LOADSWITCH_ADDRESS, 0x0F, "Button 4"},
	{10, LOADSWITCH_ADDRESS, 0x12, "Button 5"},
	{11, LOADSWITCH_ADDRESS, 0x15, "Button 6"}
};
...
int main(const kernel_t *kernel)
{
	...
	if(kernel->appSignals->appReady == true)
	{
		...
		if(buttonSwitch_0.button != button_old && buttonSwitch_0.state == buttonSwitch_state_run)
		{
			...
			tep_sendTriggerByGroup(kernel, &indexList[0], sizeof(indexList), triggerSignals, triggerSignalListSize);
			button_old = buttonSwitch_0.button;
		}
